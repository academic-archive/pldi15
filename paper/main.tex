...
amortized analysis that we implemented in $\toolname$. We then explain
and motivate our design decisions with examples.

\paragraph{Linear Potential Functions.}

To find resource bounds automatically, we first need to restrict our
search space.  In this work, we focus on a form that can express tight
bounds for many typical programs and allows inference view
\emph{linear programming}.  The general form of \emph{potential
  functions} that we consider is
$$
...
\paragraph{Examples: Challenging Loop Patterns.}
...
\paragraph{Examples: Compositionality.}
...
%
\and }{}
\Rule[leftskip=1.2cm,rightskip=.5cm]{Q:Return}
{ P = Q_R[\Vret/x]
\\ \forall i \in \dom{P} . \, p_i = q_i
}
{ B; (\Gamma_R,Q_R); (\Gamma_R[\Vret/x], Q) \vdash \code{return}~x \dashv (\Gamma',Q') }
%
\and \Rule[leftskip=.5cm,rightskip=1.2cm]{Q:Update}
{ q'_{xy}, q'_{yx} \in \Qplusz
\\ \forall u. (q_{yu} = q'_{xu} + q'_{yu} \land q_{uy} = q'_{ux} + q'_{uy})
}
{ B; R; (\Gamma[x/y], Q {+} M_u {+} M_e(y)) \vdash x \gets y \dashv (\Gamma, Q') }
...
%
\and \Rule[leftskip=1cm,rightskip=.5cm]{Q:DecP}
{ \Gamma \models y \ge 0
\\ \mathcal U = \left\{ u \mid \Gamma \models x - y \in \inter u x \right\}
\\\\ \textstyle q'_{y0} = q_{y0}
      + \sum_{u \in \mathcal U} q_{ux}
