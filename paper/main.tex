...
\\[.7\baselineskip]
  ...
  \caption{Derivations of bounds on the number of ticks for
    ...
    mention assertions that we actually use in the reasoning.}
  \label{fig:ex_list}
\end{figure*}
...
\and \Rule{S:Call}
{ \delta~f = (\vec y, S) \\
  ((\state, y_i \mapsto \state(x_i)), S)
    \Downarrow_{(q,q')} \code{ret}~\state'~v
}
{ (\state, r \gets f(\vec x)) \Downarrow_{M_f\cdot(q,q')}
  \code{seq}~(\state'~\backslash~(x_i))[r \mapsto v]
}
...
\Rule{L:Call}
{
  ...
  P \models P_f \, y \, (\sigma(\vec x)) \land A \\
  \forall v.\, (Q_f \, y \, v \land A \models \lambda \state.\, Q \, \sigma[r \mapsto v])
}
{ \Delta; B; R \vdash
\htriple
  { M_f + P }
