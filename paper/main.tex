...
XXX MENTION GULWANI AND KOAT XXX
...
and have to inline the procedure body to perform
their analysis.  Third, when such a tool fails on
program, the only way out to get a resource bound is to
rewrite the program.  There is no opportunity for sound
interaction with the tool, for example, manual proofs
cannot be composed with automatically inferred bounds.
One of the toughest challenges in the way of interaction
is that available tools work on a highly abstract version
of the source program. This abstracted program has a lot
more behaviors than the original one and is usually not
able to express memory related operations.
