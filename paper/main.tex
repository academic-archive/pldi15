...
% the semantics and the description of our analysis.  
The mutable state $\state$ can be accessed and modified, the access is
written functionally and is defined by
$$
(\theta, \gamma)(x) =
\left\{
\begin{array}{lr}
...
\right..
$$
For a program state $\state$, we write $\state[x \mapsto v]$ for the
program state that maps $x$ to $v$ and behaves as $\state$ for all
other variables, regardless whether $x \in \dom \theta$ or $x \in \dom
\gamma$.  Similarly, we just write $\state(x)$ to obtain the value of
$x$ in program state $\state$.
...
consumed by a corresponding operation.  If one of these numbers is
negative, it means that resources are released by the program.  The
metric that we use in the implementation can also depend on other
information that is statically available such as the name of the called
function or the number of arguments.
...
\pref{fig:opsem} contains the reduction rules of the semantics.
The rules define a rewrite system for program configurations of the
form $(\state, S, \cont, \cost)$, where $\state$ is the program state,
$S$ is the statement being executed, $\cont$ is a continuation that
describes what remains to be done after the execution of $S$, and
$\cost$ is the non-negative number of resources available for further
execution.  All rules that can decrease $\cost$ have the implicit side
condition that the resource quantity available after the step is
non-negative.  
...
\cost) \smallstep{M}^* (\state', S', \cont', \cost')$ is the following.
If the statement $S$ is executed in program state $\state$, with
continuation $\cont$, and with $\cost$ resources available
then---after a finite number of steps---the evaluation will reach the
new machine state $(\state', S', \cont')$ and there are $\cost'$
resources available.  The resource consumption of the evaluation up to
this point is then $\cost - \cost'$.  If this difference is negative
then resource became available during the execution.
...
\label{lem:sem1}
  \begin{enumerate}
  \item If $(\state, S, \cont, \cost) \smallstep{M}^* (\state', S',
    \cont', \cost')$ and $n\ge 0$ then $(\state, S, \cont, \cost {+} n)
    \smallstep{M}^* (\state', S', \cont', \cost' {+} n)$.
  \item If $(\state, S, \cont, \cost) \smallstep{M}^* (\state', S',
    \cont', \cost')$ and $(\state, S, \cont, d) \smallstep{M}^* (\state', S',
...
\begin{lemma}
 ...
 (\state', S', \cont', \cost') \implies \cost {-} \cost' {\leq} b$.  Then
 $(\state, S, \cont, d) \smallstep{M}^* (\state'', \code{skip}, \Kstop,
 d')$ for some $d$, $d'$ and $\state''$.
\end{lemma}
...
$$
The assertion $P_f \, z \, \vec v$ is the precondition of the
function $f$ and the assertion $Q_f \, z \, v$ is its postcondition.
They are both parameterized by an arbitary logical variable $z$
(which can be a tuple) that helps relating them.  The precondition
is allowed to depend on $\vec v$, the value of all the arguments at
the function invocation.  Similarly, the postcondition depends on
the return value $v$ of the function.


% The continuation-based semantics of a Clight requires that we
% distinguish pre- and postconditions in the   This is approach is
% standard in Hoare logics and followed for instance in Appel's
% separation logic for Clight~\cite{AppelLogic}.  Our postconditions
% $$
% Q = (Q^s, Q^b, Q^r) : \Assn \times \Assn \times (\Val \to \Assn)
% $$
% provide one assertion $Q^s$ for the case in which the block is exited by
% fall through, one assertion $Q^b$ if the block is exited by a \code{break}, and a
% function $Q^r$ from values to assertions in case the block is exited by a
% \code{return}.  The function argument in the last case represents the return
% value and the intended meaning is that the resulting assertion is
% guaranteed to hold for every return value.

% Since we have to deal with recursive functions, we also need a
% function context
% $$\Gamma {:} \funs {\to} ((\Val \times \Mem) {\to} \N \cup \{\infty\}) {\times} ((\Val \times \Mem) {\to} \N {\cup} \{\infty\})$$ 
...
% depends on the return value and the memory.  For simplicity, we assume
% that a function has only one argument in this article.  In the Coq
% implementation, an arbitrary number of function arguments is allowed.

% In summary, a quantitative Hoare triple has the form
% $$
% \Gamma \vdash \htriple{P}{S}{Q}
% $$
% where $\Gamma$ is a function context, $P : \Assn$ is a precondition,
% $Q : \Assn \times \Assn \times (\Val \to \Assn)$ is a postcondition,
% and $S$ is a statement.

% Intuitively, an assertion can be seen as a \emph{potential function}
% that maps a program state to a non-negative potential.  The potential
% of the precondition $P$ must be sufficient to cover the cost of the
% execution of the statement $S$ and the potential $Q$ after the execution
% of $S$ (as in amortized resource analysis~\cite{HoffmannAH12}).
...
\Rule{Q:Skip}
{}
{ \Gamma; B; R; Q \vdash \code{skip} \dashv Q; \Gamma }
%
\and \Rule{Q:Break}
{}
{ \Gamma; B; R; B + M_b \vdash \code{break} \dashv Q'; \Gamma' }
%
\and \Rule{Q:Return}
{}
{ \Gamma; B; R; R \, x + M_r \vdash \code{return}~x \dashv Q'; \Gamma' }
%
\and \Rule{Q:Assert}
{}
{ \Gamma; B; R; Q + M_a \vdash \code{assert}~e \dashv Q; \Gamma, e }
%
\and \Rule{Q:Loop}
{ \Gamma; Q'; R; Q \vdash S \dashv M_l + Q; \Gamma }
{ \Gamma; B; R; Q \vdash \code{loop}~S \dashv Q'; \code{inv}_S~\Gamma }
%
\and \Rule{Q:Set}
{ q'_{xy}, q'_{yx} \in \mathbb Q_0^+
\\ \forall u. (q_{yu} = q'_{xu} + q'_{yu} \land q_{uy} = q'_{ux} + q'_{uy})
}
{ \Gamma[x/y]; B; R; Q + M_s \vdash x \gets y \dashv Q'; \Gamma }
%
\and \Rule{Q:IncrPos}
{ \Gamma \models 0 \le y
...
{ \textstyle
  \Gamma[x/x + y]; B; R; Q + M_s \vdash x \gets x + y \dashv Q'; \Gamma
}
%
\and \Rule{Q:IncrNeg}
{ \Gamma \models 0 \le y
...
{ \textstyle
  \Gamma[x/x \pm y]; B; R; Q + M_s \vdash x \gets x \pm y \dashv Q'; \Gamma
}
%
\and \Rule{Q:If}
{ \Gamma, e; B; R; Q - M_c^1 \vdash S_1 \dashv Q'; \Gamma'
\\ \Gamma, \neg e; B; R; Q - M_c^2 \vdash S_2 \dashv Q'; \Gamma'
}
{ \Gamma; B; R; Q \vdash \code{if}(e)~S_1~\code{else}~S_2 \dashv Q'; \Gamma' }
%
\and\Rule{Q:Seq}
{  \Gamma; B; R; Q - M_s^1 \vdash S_1 \dashv Q'; \Gamma'
\\ \Gamma'; B; R; Q' - M_s^2 \vdash S_2 \dashv Q''; \Gamma''
}
{ \Gamma; B; R; Q \vdash S_1;S_2 \dashv Q''; \Gamma'' }
%
\and\Rule{Q:Call}
{ (\vec y, s, \Gamma_f, P, P', \Gamma_f') \in \Delta(f)
\\ \ell = \text{Locals}(Q)
\\ R = P[\vec y / \vec x]
\\ \forall i \in \dom{R} . \, q_i = r_i
\\ R' = P'[s/r]
\\ \forall i \in \dom{R'} .\, q'_i = r'_i
\\ \forall i \not\in \dom{R'} .\, q'_i = 0
\\ \forall i \in \ell . \, q_i = q_i'
% \\ P' = Q'_f \Uparrow_{[r = s]}
% \\ \forall i \in \ell \,\setminus \{r\} .\, q_i = q_i'
% \\ \forall i \not\in \ell \,\setminus \{r\} .(q_i = p_i \land q'_i = p'_i)
% \\ c \in \mathbb Q_0^+
}
{ \Gamma_f[\vec y / \vec x] \land \Gamma_\ell; B; R; Q + c
  \vdash r \gets f(\vec x) \dashv
  Q' + c; \Gamma_f'[s/r] \land \Gamma_\ell
}
%
\and \Rule{Q:Weak}
{ \Gamma_1 \models \Gamma_2
\\ Q_1 \succeq_\Gamma Q_2
\\ \Gamma_2; B; R; Q_2 \vdash S \dashv Q'_2; \Gamma'_2
\\ Q'_2 \succeq_\Gamma Q'_1
\\ \Gamma'_2 \models \Gamma'_1
}
{ \Gamma_1; B; R; Q_1 \vdash S \dashv Q'_1; \Gamma'_1 }
%
\and \Rule{Relax}
