...
while (y-x>K) {
  ${\color{blue}%
  x{<}y; \; 0 + \frac{T}{K} {\cdot} |[x,y]| + 0 {\cdot} |[y,x]| \vdash }$
  x=x+K; 
  ${\color{blue}%
...
computes the bound XXX, PUBS~\cite{AlbertAGPZ12} computes the bound
XXX, Rank~\cite{AliasDFG10} computes the bound XXX, and
LOOPUS~\cite{SinnZV14} computes the bound XXX.
