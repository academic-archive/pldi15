...
\\ \llbracket e \rrbracket_H = \code{ptr}~b
\\ ({\rm alloc}(H, a_i, \llbracket e_i \rrbracket_H), S) \Downarrow_{(q,q')} \code{ret}~H'~v
}
{ (H, e \gets f(e_1, \dots, e_n)) \Downarrow^r_{M_f\cdot(q,q')} \code{seq}~{\rm free}(H',a_i)[b \mapsto v] }
