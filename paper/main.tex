...
\Rule{S:RetSeq}
{}
{ (\state, \code{return}~e, \Kseq S \cont, \cost)
\smallstep
(\state, \code{return}~e, \cont, \cost)
}
\and
%
\and
\Rule{S:RetLoop}
{}
{ (\state, \code{return}~e, \Kloop \cont, \cost)
\smallstep
(\state, \code{return}~e, \cont, \cost)
}
%
...
{ (\state, \code{return}~e, \Kcall r \theta \cont, \cost)
\smallstep
...
$(\state, S) \Downarrow_{(q,q')} \circ$ covers unsafe programs, diverging
...
{ \forall f\,P_f\,Q_f.\,
  ...
  (\Delta \cup \Delta'; \bot; Q_f\,y \vdash
    \htriple{ P_f\,y\,\vec v }{ \delta f }{ \bot })
\\
