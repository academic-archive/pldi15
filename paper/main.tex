...
program state such that $(S, \state) \Downarrow_n \state'$.
...
pre- and postconditions to take non-negative number or $\infty$
...
\end{center}
%
It holds for all stack metrics $M$.
...
is, we use a resource metric that assigns cost $n$ to the function call
...
at the assignment if we can extract constant potential to pay for future
...
that consumes $T>0$ resources in each iteration.  The reasoning is similar
...
of track more sophisticated assertions.  However, this simple form is
