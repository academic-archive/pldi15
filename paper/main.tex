...
\begin{abstract}
  ...
  for C programs computes derivations in the quantitative Hoare logic.
...
track of more sophisticated assertions.  However, this simple form is
...
work~\cite{GulwaniMC09} and demonstrate that our method can handle
...
its counter instrumentation.  It can also be handled with inference of
...
manually find a bound.  However, using potential transfer reasoning as
...
function pointers, \code{goto} statements, \code{continue} statements,
and \code{switch} statements.
...
simplified version of Clight's semantics for the subset we are
describing.
