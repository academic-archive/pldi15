...
\emph{speed\_2} in Figure~\ref{fig:ex_list_2}, which are taken from previous work~\cite{GulwaniMC09},
...
routines of PGP.  The same pattern is also used in performance critical
code to unroll a loop, for example we found it in a bit manipulating
function of libtiff library and a CRC computation routine of MAD,
an MPEG audio decoder.  This looping pattern is handled particularly well
...
\log_2(h-l)$ as invariant.  This technique allows a simpler
...
demonstrates the compositionally of the analysis.  The program consists
...
more than 2000 lines of code.  In the LoC column
...
The functions that took the longest to analyze are often unrolled loops
...
than 2 seconds.  These times can even be greatly reduced by adding
...
The trickiest of these benchmarks is the \code{sha\_update} function,
it is composed of one loop calling two helper functions that in turn
have 6 and 1 inner loops.  The essence of the SHA algorithm makes
the compositionality of our analysis essential to get a tight bound
since loops on the same index are sequenced 4 and 2 times without
resetting it.  All other tools available have constant factors
quadratically bigger than ours on this example.
...
\end{tabular}
\vspace{.1cm}
\caption{Derived bounds for functions from cBench. \jan{FUCKING backedge metric}}
\label{tab:cbench}
\end{table}
...
technical innovations are implemented in the publicly available tool
...
amortized resource analysis to compute bounds for programs that loop
on signed integers and to deal with non-linear control flow that is
