...
\begin{abstract}
  ...
  %
  The analysis framework is implemented in the publically-available
  ...
  more than 2000 lines of C code from the cBench benchmark suite.
...
that amortizes the cost of $k$ successive increments to a binary
...
\item We present experiments with a prototype implementation on more
  than 2000 lines of C code. A detailed comparison shows that our
...
programs and allows inference view \emph{linear programming}.
$$
...
\paraskip
\paragraph{Mutual Recursive Functions.}
...
c up $y$ by $2$ and recursively call \code{c\_down}.  $\toolname$ is
...
Example \emph{t61} in \pref{fig:ex_list_2} is typical for implementations of block-based
cryptographic primitives: Data of arbitrary length, is consumed in
blocks and the leftover is stored in a buffer for future use when more
data is available.  This looping pattern is particularly well handled
by our method. If $N \geq 8$, $\toolname$ infers the bound
$\frac{N}{8} |[0,l]|$, but if $N<8$ $\toolname$ infers $7\frac{8-N}{8}
+ \frac{N}{8}|[0,l]|$. The selection of the block size ($8$) and the
cost in the second loop (\code{tick(1)}) are random choices and
$\toolname$ would also derive tight bound for other values.
...
Most interesting are the rules {\sc Q:IncP}\iffull{, {\sc Q:DecP},}{} and {\sc Q:Inc}
...
Finally, the rule {\sc Q:Inc} can be applied
...
derivation rules of the automatic amortized analysis.  During this
...
The rules are syntax directed and applied inductively.  For example,
...
$$}{.}
%
This constraint expresses the fact that the potential before and
...
used to gather information on interval sizes.  For example, we have
$$
\begin{array}{lll}
...
so-called objective function to minimize.  We wish to
minimize the initial potential, which constitutes the resource bound on the
...
the linear expression \ifshort{$L := 1{\cdot}q_{x,0} + 10000{\cdot}q_{0,x}
+ 11{\cdot}q_{x,10} + 9990{\cdot}q_{10,x}$.}{
$$
L := 1{\cdot}q_{x,0} + 10000{\cdot}q_{0,x} + 11{\cdot}q_{x,10} + 9990{\cdot}q_{10,x} \; .
$$}
The penalties given to certain factors are used
to prioritize certain intervals.  For example, a bound with
$[10,x]$ will be prefered to another with $[0,x]$ because $|[10,x]|
\le |[0,x]|$.
%
...
\jan{ the following should go somewhere here:

Note that potential functions can access the whole program state, so
this framework can readily be used to derive bounds that depend on the
memory contents.
}
...
our cost-aware semantics.  Some rules are presented in \pref{fig:logic}.

Finally, still following previous work, we define the a continuation based
soundness of triples and prove the soundness of all rules in Coq.  The
full version of this paper \cite{anon_extended}, gives more explanations
on the rules and provides a thorough overview of our soundness proof.

\subsection{The Soundness Theorem}

We use the
logic described in the previous paragraph as the target of a translation
function for the automatic derivation system.  This reveals two orthogonal
aspects of our proof that relies, on one side, on amortized reasoning
(as encoded in the quantitative logic), and on the other side, on
combinatorial properties of the linear potential functions that we
are using (as encoded in the automatic analysis).
