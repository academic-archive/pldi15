...
that can be applied if $y$ is not positive.
...
the iterations.  At each iteration, $M_l$ resources are spent to jump
back, this explains the postcondition $Q{+}M_l$.  To exit the loop,
the programmer uses the \code{break} statement, it is the reason why
the postcondition for the full $\code{loop}~S$ statement $\{ \Gamma';
Q' \}$ is used as break postcondition in the derivation for $S$.
...
The rule {\sc Q:Weak} is the only rule not syntax directed. %In the implementation
...
larger than a constant $l$ then we can turn the potential $q_{xy} {\cdot}
|\inter x y|$ form the interval into the constant potential $l{\cdot}q_{xy}$
