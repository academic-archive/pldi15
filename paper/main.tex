...
The most interesting rules are {\sc Q:IncP}, {\sc Q:DecP}, and {\sc
  Q:Inc}.  

The case of the update statement is more interesting and specific to
linear potential functions.  In the {\sc Q:IncP} rule the program
updates a variable $x$ to add it $y \ge 0$.  Since $x$ is changed, the
...
($x + y$). Since $x$ is the only variable changed, only intervals of
the form $\inter u x$ and $\inter x u$ will be resized.  Note that for
any $u$, $\inter x u$ will get smaller with the update, and if $x' \in
\inter x u$ we have $|\inter x u| = |\inter x {x'}| + |\inter {x'}
u|$.  But $|\inter x {x'}| = |\inter 0 y|$, this means that the
potential $q'_{0y}$ in the postcondition can be increased by $q_{xu}$
under the guard that $x' \in \inter x u$.  Dually, the interval
$\inter v x$ can get bigger with the update.  And we know that
$|\inter v {x'}| \le y + |\inter v x|$, so we decrease the potential
of $\inter 0 y$ by $q_{vx}$ to pay for this change.  Remark that the
rule forces it only for $v \not\in \mathcal U$, because if not, we
know that $x \le v$ so $|\inter v x| = 0$.
...
All rules except of {\sc Q:Weak} and {\sc Q:Relax} are syntax
directed.  
...
\begin{tabular}{r|p{3cm}c|p{3cm}c|c}
...
%--------------------------------------------
\texttt{speed\_pldi09\_fig4\_2.c} &
...
%--------------------------------------------
\texttt{speed\_pldi09\_fig4\_4.c} &
...
%--------------------------------------------
\texttt{speed\_pldi09\_fig4\_5.c} &
...
%--------------------------------------------
\texttt{speed\_pldi10\_ex1.c} &

$(((1+1)+n)+((-1+((1/2){\cdot}n))+((1/2){\cdot}(n{\cdot}n))))+1$ &
$O(n^2)$ &

$\max(n, 0) + \max(0,  \max(n, 0) {\cdot} \max(0, (n + -1)))$ &
$O(n^2)$ &
...
%--------------------------------------------
\texttt{speed\_popl10\_fig2\_1.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_fig2\_2.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_nested\_multiple.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_nested\_single.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_sequential\_single.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_simple\_multiple.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_simple\_single\_2.c} &
...
%--------------------------------------------
\texttt{speed\_popl10\_simple\_single.c} &
...
$|\inter x 0| + |\inter 0 y| + 1002 |\inter y x|$
\\
