...
\authorinfo
{\iffull{Quentin Carbonneaux \and Jan Hoffmann \and Zhong Shao}{\vspace{-1cm}}}
{\iffull{Yale University}{\vspace{-1cm}}}
...
flow that depends integer arithmetic, negative numbers, and non-linear
control flow.
% example, it is a
...
asymptotically tight bounds for all but 3 of these patterns and in
...
with the tools such as KoAT~\cite{BrockschmidtEFFG14},
...
interested in bounding the number of \emph{ticks} in a program.  Thus,
we use a resource metric that assigns cost $n$ to the function call
...
a program state $\state$ simply maps variable to integers then we
require
$$
...
the cost of the following example can be bounded by $|[x,y]|$. Note
...
variables for each factor in the potential functions.  We then use our
inference rules (see \pref{sec:AAA}) to emit linear inequalities on
these variables that enforce that the potential annotations are sound.
For instance if $K = 1$ in \pref{fig:ex1} then we would have the
...
work~\cite{GulwaniMC09} and demonstrate the our method can handle
\emph{tricky iteration patterns}.  The SPEED tool~\cite{GulwaniMC09}
derives the same bounds as our analysis but requires inference of
\emph{disjunctive invariants}.  In the abstract interpretation
...
manually find a bound.  However, using potential transfer reasoning of
amortized analysis, it is straightforward to prove the tight bound
$|[x, n]| + |[z, n]|$.

Example \emph{t08a} shows the ability of the analysis to discover
interaction between \emph{sequential loops} through size changes of
variables.  We accurately track the size change of $y$ in the first
loop by transferring the potential $0.1$ form $|[y,z]|$ to $|[0,y]|$.
...
variable $n$ till $n = 0$.  In each of the $|[n,0]|$ iterations, we
increment the variable $y$ by $1000$.  We then non-deterministically
...
\right..
$$}{}
\iffull{For a program state $\state$, we}{We} similarly write $\state[x \mapsto v]$
...
{
The extended version of this article~\cite{anon_extended} contains all rules lemmas 
that state the main properties of the cost semantics.
}
...
evaluation of the argument variables in the current state.
\iffull{Recall that we require that $P_f$ and $Q_f$ do not depend on
  the local variables on the call stack, that is, $\forall z \, \vec v
  \, \theta \, \theta' \, \gamma . \, P_f \, z \, \vec v \,
  (\theta,\gamma) = P_f \, z \, \vec v \, (\theta',\gamma)$.}{} To
transfer potential that depends on local variables of the callee from
the precondition $P$ to the postcondition $Q$, we use an assertion $A
: \Assn$ that is independent of global variables, that is, $\forall
\theta \, \gamma \, \gamma' . \, A (\theta,\gamma) =
A(\theta,\gamma')$.  It is still possible to express relations between
global and local variables using logical variables (see the
\iffull{following paragraph}{extended version} for details).
...
number of times the test \lstinline{a[i] > m} is true during the
execution.
%
If we define
$$
...
the number of ``maximum candidates'' in the array \lstinline{a} seen
...
When the condition \lstinline{a[i] > m} is true, we know that we
encountered a ``maximum so far'' because $m$ is a maximal element of
\lstinline{a[$0 \dots i$]}, thus $A(i) = 1 + A(i+1)$ and we get one
...
index $(x,y)$.  The family $(f_i)_{i\in I}$ is actually a basis (in
...
{\sc Q:IncP} is for increments $x \gets x + y$ in which we can deduce
from the logical context $\Gamma$ that $y$ is not negative.
...
$q_{vx}$ to pay for this change.  The rule insures this
...
The rule {\sc Q:Relax} deals also with the important task of transferring
...
the symbolic names.  When then feed the collected constraints to an
...
to deal with resources like memory usage.  Blazy et al.'s automatic
bound analysis does also not compute symbolic bounds.
...
We will continue to improve our framework to reasoning more precisely
...
CertiKOS~\cite{GuVFSC11}, which \iffull{we are currently developing
  and verifying}{is currently developed and verified}.  For on thing,
...
\code{count\_down}.  For the function $\main$, which calls
...
Again, we use the \emph{tick metric} and the function \code{\tick} to
