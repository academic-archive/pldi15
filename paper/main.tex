...
a value $\llbracket e \rrbracket_H$ using $H$ as current
heap.

\section{Semantics}

Values computed by evaluating expressions can be integers, or
memory locations. In our semantics, integers have infinite
precision.  While the program logic would perform as well on
machine integers, our automatic analysis would be unsound.
%
\begin{align*}
V &:= \code{int}~i \mid \code{ptr}~b
\end{align*}

To formally describe the cost of operation of a given
program, our semantics tracks resource consumption while
executing.  To account for all kinds of resources
...
\and \Rule{S:Assert}
{ {\rm istrue}~\llbracket e \rrbracket_H}
{ (H, \code{assert}~e) \Downarrow_{M_a} \code{seq}~H }

\and \Rule{S:Skip}
{ }
{ (H, \code{skip}) \Downarrow_{M_s} \code{seq}~H }

\and \Rule{S:Break}
{ }
{ (H, \code{break}) \Downarrow_{M_b} \code{brk}~H }

\and \Rule{S:Return}
{ }
{ (H, \code{return}~e) \Downarrow_{M_r} \code{ret}~H~\llbracket e \rrbracket_H }

\and \Rule{S:Update}
{ \llbracket e \rrbracket_H = \code{ptr}~b }
{ (H, e \gets e') \Downarrow_{M_u} \code{seq}~H[b \mapsto \llbracket e' \rrbracket_H] }

\and \Rule{S:FunCall}
{ \Delta~f = ((a_1, \dots, a_n), S)
\\ \llbracket e \rrbracket_H = \code{ptr}~b
\\ (H +\!\!\!+ (a_i \mapsto \llbracket e_i \rrbracket_H)_i, S) \Downarrow_{(q,q')} \code{ret}~H'~v
}
{ (H, e \gets f(e_1, \dots, e_n))
\Downarrow^r_{M_f\cdot(q,q')} \code{seq}~(H'\,\backslash\,(a_i)_i)[b \mapsto v] }

\and \Rule{S:LoopSeq}
{ (H, S) \Downarrow_{(q_1, q_1')} \code{seq}~H'
\\ (H', \code{loop}~S) \Downarrow_{(q_2, q_2')} X }
{ (H, \code{loop}~S) \Downarrow_{(q_1, q_1') \cdot M_l \cdot (q_2, q_2')} X }

\and \Rule{S:LoopBrk}
{ (H, S) \Downarrow_{(q, q')} \code{brk}~H' }
{ (H, \code{loop}~S) \Downarrow_{(q, q')} \code{seq}~H' }

\and \Rule{S:LoopAbort}
{ (H, S) \Downarrow_{(q, q')} X
\\ \forall H.~X \neq \code{seq}~H \land X \neq \code{brk}~H }
{ (H, \code{loop}~S) \Downarrow_{(q, q')} X }

\and \Rule{S:IfThen}
{  (H, S_1) \Downarrow_{(q, q')} X
\\ {\sf istrue}~\llbracket C \rrbracket_H
}
{ (H, \code{if}(C)~S_1~\code{else}~S_2) \Downarrow_{M_c^1 \cdot (q, q')} X }

\and \Rule{S:IfElse}
{  (H, S_2) \Downarrow_{(q, q')} X
\\ {\sf isfalse}~\llbracket C \rrbracket_H
}
{ (H, \code{if}(C)~S_1~\code{else}~S_2) \Downarrow_{M_c^2 \cdot (q, q')} X }

\and \Rule{S:Seq}
{  (H, S_1) \Downarrow_{(q_1, q'_1)} \code{seq}~H'
\\ (H', S_2) \Downarrow_{(q_2, q'_2)} X
}
{ (H, S_1; S_2) \Downarrow_{(q_1, q'_1) \cdot M_q \cdot (q_2, q'_2)} X }

\and \Rule{S:SeqAbort}
{ (H, S_1) \Downarrow_{(q,q')} X
\\ \forall H.~X \neq \code{seq}~H}
{ (H, S_1; S_2) \Downarrow_{(q,q')} X }
