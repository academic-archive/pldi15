...
Moreover, C is also a natural choice because it is still the most
widely used language for system development; in particular in
safety-critical embedded and real-time systems where resource bound
analyses are often required by regulatory authorities~\cite{May2013}.
...
part of the stack-bound verification is based on a quantitative Hoare
...
to automatically derive stack bounds C programs without recursion such
as CertiKOS.
...
for Clight that counts a constant resource consumption for each
statement.  The resource consumption is defined by a cost metric and can be
negative to express the release of resources (e.g, as needed to reason
about memory usage).  The semantics models cost of terminating and
non-terminating executions and a specific \emph{tick} function can be
inserted in the source code to further customize the cost model.



- Problem: very tedious to reason about large programs
- automatic amortized analysis was inspiration for logic
- but hard to apply to C programs
- main problem: negative integers and precise bounds for \emph{for loops} like in python
  and control C-like control-flow with break and return

Our \emph{second contribution} is 
