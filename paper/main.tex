...
Static analysis of quantitative properties of imperative
programs is an active research area and recent years
have seen many innovations.  Notable tools that
have been developed include SPEED~\cite{GulwaniMC09},
KoAT~\cite{BrockschmidtEFFG14}, PUBS~\cite{AlbertAGPZ12},
Rank~\cite{AliasDFG10}, and LOOPUS~\cite{SinnZV14}.
These tools can derive impressive results on realistic
system software.  However, there are still theoritical
shortcomings that remain open problems to this day:

\begin{itemize}
\item The computed bounds are often
  \emph{non-compositional} local bounds (for a single
  (nested) loop) that are difficult to combine to
  global whole program bounds.
\itemskipIn
\item When an automatic tool fails on a loop of the
  program, no user interaction is possible to derive
  a bound on the full program.
\itemskipIn
\item Existing tools work on a highly abstracted
  version of the program, thus, are unable to provide
  a simple syntactic proof of the correctness of their
  bound.
\itemskipIn
\item Resources that can be freed like memory cannot
  be handled with the usual techniques.
\end{itemize}

This paper presents a new framework to derive worst-case
resource bounds for C programs.  Our choice of C is
mainly motivated by its strong presence in system and
embedded software.  These applications often have to meet
strict time and memory constraints.

XXX [this has nothing to do here]    To ensure that these
requirements are met, profiling is not enough because non
exhaustive.  In contrast, static analysis tools provide
stronger and sound guarantees.

Our compositional framework is built of three parts: 1-
a clear cost model for C programs based on instrumented
semantics; 2- a quantitative Hoare logic allowing
compositional reasoning on the resource consumption of
C programs at a syntactic level; 3- an automated tool
that derives bounds in the previous logic.

Compared to existing work, our framework is a jump in
compositionality and correctness: the analysis is no
longer centered on single loops but on complete programs,
a mix of automation and interaction can be used to derive
global bounds, and bounds are computed and proved within
a clear syntactic deductive system.  Compositionality is
due first to the logic that soundly allows mixes of hand
derived bounds and automatically generated bounds.  It is
also due to the automated tool that keeps track of loop
bounds and size changes of variables in a unified system.

The cost model and the quantitative Hoare logic
presented are based on a recent work~\cite{veristack14}
that formally verifies bounds on the stack usage of
C programs.  We generalize this work by allowing
to reason on any resource (e.g. evaluation steps,
memory, or stack usage).  The automation, which is the
keystone of this paper, is based on amortized analysis
techniques~\cite{jan-something}.  It makes the analysis
of large programs tractable by automatically generating
resource bounds that are certified in the previous logic
on a large class of programs.

Our automated analysis is able to infer linear
resource bounds on C programs with integer loops.
To our knowledge this is the first tool based on
amortized analysis able to deal with negative numbers and
differences of variables.  It is also the first resource
analysis tool for C that deals naturally with recursive
functions, resources that can be freed (e.g. memory),
and sequenced loops.  Automated tools for functional
programs already existed~\cite{Jost03,HoffmannAH12}
but it has been a long-time open problem to apply it to
C-like programming languages with control flow depending
on integer arithmetic.

Compared to more classical approaches based on ranking
functions, tools that are based on amortized analysis
only need one mechanism to provide features that are
usually treated non uniformly:

\begin{itemize}
\item interaction between \emph{sequential loops or
...
\item and \emph{amortized bounds} as found
  in the Knuth-Morris-Pratt algorithm for string
  search.
\end{itemize}

The main innovation that makes amortized analysis
work on imperative languages is to track multivariate
quantites instead of simple functions of program
variables.  This leads to precise bounds expressed as
functions of sizes $|[x,y]| = \max(0,y-x)$ of intervals.

This paper presents a compositional framework for
deriving certified resource bounds on C programs. We
emphasize the following contributions: 1- an automatic
analysis that generates linear compositional and
certified resource bounds; 2- an infrastructure verified
in Coq that lets a programmer derive complex free-form
resource bounds on complete C programs.
