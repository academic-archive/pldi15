...
\newcommand{\itemskip}[0]{}
\newcommand{\itemskipIn}[0]{}
\newcommand{\sectskip}[0]{}
\newcommand{\paraskip}[0]{}
\newcommand{\lstskip}[0]{}
\newcommand{\aftersectskip}[0]{}
\newcommand{\subsectskip}[0]{}
...
\usepackage{flushend} %equalize columns length on last page
\usepackage{hhline}
%\usepackage[firstpage]{draftwatermark}
%\SetWatermarkText{\hspace*{8in}\raisebox{5.5in}{\includegraphics[scale=0.1]{fig/aec-badge-pldi.pdf}}}
%\SetWatermarkAngle{0}
...
with individual program variables or data structures.
For C programs, this
...
in a functional specification that can be used at every
...
recursive functions and sequenced loops, and can handle resources that may
...
\item We develop the first automatic amortized analysis for C
  ...
  proof certificates, and handles resources that may
...
Hoare logic) to derive a triple
...
${\color{blue}%
  \{ \cdot; \, 0 + \frac{T}{K} {\cdot} |[x,y]| \} }$
while (x+K<=y) {
  ${\color{blue}%
  \{ x + K \le y; \; 0 + \frac{T}{K} {\cdot} |[x,y]| \} }$
  x=x+K;
  ${\color{blue}%
  \{ x \leq y; \, T + \frac{T}{K} {\cdot} |[x,y]| \} }$
  tick(T);
  ${\color{blue}%
  \{ x \leq y; \, 0 + \frac{T}{K} {\cdot} |[x,y]| \} }$
}
${\color{blue}%
 \{ x \geq y; 0 + \frac{T}{K} {\cdot} |[x,y]| \} }$
\end{lstlisting}
...
$K{\cdot}\frac{T}{K}$ after the assignment.  In the figure, we separate
logical assertions from potential functions with semicolons. Note that the
logical assertions are only used in the rule for the assignment
\code{x=x+K}.
...
constant $1988$ then we have a variable $c_{1988}$ and
$\state(c_{1988}) = 1988$.  We assume that every program state
...
the rules of the analysis often require only minimal local knowledge.
...
loop has linear complexity, and feeding it to our analysis gives the
worst-case bound $2+3|[l,h]|$.  This bound is not optimal but it can be
refined by rewriting the program. % to let our logical inference
%discover more invariants.
%With the potential
%method in mind, it is clear that the three loops get their
%potential from the same interval $[l,h]$ regardless of their nesting.
To understand the bound, we can reason as follows.
%
If $h \geq l$ initially, the cost of the loop is 2.
Otherwise, the cost of each round (at
most 3) can be payed using the potential of $[l,h]$ by the first
...
\paraskip
\paragraph{Cost Aware Clight.}

We present the rules for a subset of Clight.
Clight is the first intermediate
language of the CompCert compiler~\cite{CompcertCACM}. It
is a subset of C with a unified looping construct and
side-effect free expressions.  We reuse most of CompCert's
syntax but instrument the semantics with a resource metric
$M$ that accounts for the cost (an arbitrary rational number)
of each step in the operational semantics.  For example, $M_e(\code{exp})$ is the cost of
evaluating the expression \code{exp}. The rationals $M_f$
and $M_r$ account respectively for the cost of a call to the
function $f$ and the cost of returning from it.
More details are provided in \pref{sec:soundness}.
...
returning from a function call. \iffull{(This becomes clear
in the rules.)}{}
...
\and \Rule{Q:Assert}
{ }
{ B, R \vdash \htriple{ \Gamma; Q {+} M_a }{ \code{assert}~e }{ \Gamma {\land} e; Q } }
%
\and \Rule{Q:Tick}
{ }
{ B, R \vdash \htriple{ \Gamma; Q {+} M_t(n) }{ \code{tick}(n) }{ \Gamma; Q } }
%
\and \Rule{Q:Break}
{ }
{(\Gamma; Q_B), R \vdash \htriple{ \Gamma; Q_B {+} M_b }{ \code{break} }{ \Gamma'; Q' } }
%
\and \Rule{Q:Return}
{ P = Q_R[\Vret/x]
\\ \Gamma = \Gamma_R[\Vret/x]
...
{ B, (\Gamma_R; Q_R) \vdash \htriple{ \Gamma; Q }{ \code{return}~x }{ \Gamma'; Q' } }
%
\and \Rule{Q:Loop}
...
%
\and \Rule{Q:If}
{ B, R \vdash \htriple{ \Gamma {\land} e; Q {-} M_c^1) }{ S_1 }{ \Gamma'; Q' }
\\\\ B, R \vdash \htriple{ \Gamma {\land} \neg e; Q {-} M_c^2 }{ S_2 }{ \Gamma'; Q' }
}
{ B, R \vdash \htriple{ \Gamma; Q{+}M_e(e) }{ \code{if}(e)~S_1~\code{else}~S_2 }{ \Gamma'; Q' } }
%
\\ \Rule{Q:IncP}
{ \Gamma \models y \ge 0 \!
\\ \mathcal U = \left\{ u \mid \Gamma \models x + y \in \inter x u \right\} \!
...
{ B, R \vdash \htriple{ \Gamma[x/y]; Q {+} M_u {+} M_e(y) }{ x \gets y }{ \Gamma; Q' } }
%
\and \Rule{Q:DecP}
{ \Gamma \models y \ge 0
\\ \mathcal U = \left\{ u \mid \Gamma \models x - y \in \inter u x \right\}
...
%
\and \Rule[leftskip=.2cm,rightskip=.2cm]{Q:Extend}
{ \fenv f = (\vec y, S_f)
...
the execution of a skip statement. Thus all potential available
...
The rules {\sc Q:IncP}, {\sc Q:DecP}, and {\sc Q:Inc}
describe how the potential is distributed after a size change of
a variable.
The rule {\sc Q:IncP} is for increments $x \gets x + y$
...
Of course, there are symmetrical rules {\sc Q:IncN} and {\sc Q:DecN}
(not presented here) that can be applied if $y$ is negative.
The rules are all equivalent in the case where $y=0$.
The rule {\sc Q:Inc} can be applied
if we cannot find the sign of $y$.  % In $\toolname$,
% the rules for increments and decrements are combined into one rule\iffull{ where
% the specialized rules take priority over {\sc Q:Inc}.}{.}
...
The rule {\sc Q:Loop} is a cornerstone of our analysis.  To apply it
on a loop body, one needs to find an invariant potential $Q$ that will
pay for the iterations.  At each iteration, $M_l$ resources are spent
to jump back. This explains the postcondition $Q{+}M_l$. Since the
loop can only be exited with a \code{break} statement, the
postcondition $\{ \Gamma'; Q' \}$ for the statement $\code{loop}~S$ is
used as break postcondition in the derivation for $S$.
...
prevents name conflicts and ensures that
...
The rule {\sc Q:Weak} is the only rule that is not syntax directed. %In the implementation
%we apply {\sc Q:Weak} before loops and between the two statements of a
%sequential composition.
We could integrate weakenings into every syntax directed rule but, for
the sake of efficiency, the implementation uses a simple heuristic instead.
The high-level idea of {\sc Q:Weak} is the following: If
...
that is defined in the rule {\sc Relax}. This rule
...
from the logical context that the interval size $|\inter x y| \geq \ell$ is
larger than a constant $\ell$ then we can turn the potential $q_{xy} {\cdot}
|\inter x y|$ form the interval into the constant potential $\ell{\cdot}q_{xy}$
...
$u{\cdot}q_{xy}$ to the interval potential $q_{xy} {\cdot} |\inter x y|$
without gaining potential.
...
a proof certificate instead of a counter example.  To provide
high-assurance bounds, this certificate can be checked
in linear time by a simple validator.
...
logical variables is selected to satisfy the proposition:
\begin{equation} \tag{*}
...
\lstinline{na > 0} on line 5 cannot fail.  We do it
by proving inductively that $na = \#_1(a)$
...
\log_2(h-l)$ as invariant.  This allows a simpler
...
satisfy the proposition (*).  This can usually be piggybacked
...
as $B;R\vdash_L \htriple Q S {Q'}$ where $B$, $R$, $Q$, and $Q'$ are
...
$$}{}
%
These definitions let us translate the judgement
$J := B, R \vdash \htriple P S {P'}$\iffull{ in the context $\Delta$ by $\tr(J) :=$}{ by}
\iffull{$$
  (\lambda f .\, \tr(\Delta(f))); \tr(B); \tr(R) \vdash
    \htriple{\tr(P)}{S}{\tr(P')}.
$$}{%
$$
  \tr(J) := \tr(B); \tr(R) \vdash_L \htriple{\tr(P)} S {\tr(P')}.
$$}%
...
quantitative logic.  The trickiest parts are the translations of the
...
down to $-1$.  However, between the loops we assign \lstinline{i += k+50}.
...
\begin{table}[t]
\nocaptionrule \caption{Comparison of $\toolname$ with other
  automatic tools.}
\vspace{6pt}
\label{tab:compar}
\small
...
\pref{tab:compar} summarizes the results of our experiments presented
in \pref{app:eval}.  It shows
...
With our formal cost semantics, we can run our examples
for different inputs and measure the cost to compare it
...
\begin{table}[t]
\nocaptionrule \caption{Derived bounds for functions from cBench.}
\vspace{6pt}
\label{tab:cbench}
\centering
\small
...
all of Clight.  Programs with function pointers, goto statements,
continue statements, and pointers to stack-allocated variables
cannot be analyzed automatically.  While these limitations concern the
current implementation, our technique is in principle capable to handle
them.

For the sake of simplicity, the automated system described here is
restricted to finding only linear bounds.  However, the amortized analysis
technique was shown to work with polynomial bounds~\cite{HoffmannAH11};
we leave this extension of our system as future work.

Even certain linear programs cannot be analyzed automatically by
$\toolname$, it is usually the case for programs that rely on heap
...
\code{\%} or \code{*}) in a non-trivial way, or for programs whose
termination can only be proved by complex path-sensitive reasoning.
...
recursive functions fully automatically. We
use a more general quantitative Hoare
logic that is parametric over the resource of interest.
...
the aforementioned tools. Our technique, is the only one that can
generate resource specifications for functions, deal with resources
like memory that might become available, generate proof certificates
for the bounds, and  support user guidance that separates
...
techniques with the potential method of amortized analysis.  It is
implemented in the publicly available tool $\toolname$. To the best of
our knowledge, $\toolname$ is the first tool for C programs
that automatically reduces the derivation of symbolic bounds to LP
solving.
...
We thank members of the FLINT team at Yale and anonymous referees for
helpful comments and suggestions that improved this paper and the
implemented tools.  This research is based on work supported in part
by NSF grants 1319671 and 1065451, DARPA grants FA8750-10-2-0254 and
FA8750-12-2-0293, and ONR Grant N00014-12-1-0478.  Any opinions,
findings, and conclusions contained in this document are those of the
...
\begin{table*}[h!]
\appendix
\section{Complete Experimental Results for the Tool Comparison}
\label{app:eval}

\renewcommand{\max}[0]{{\rm mx}}
\nocaptionrule \caption{
  Comparison of the bounds generated by KoAT, Rank,
  LOOPUS, SPEED, and our tool $\toolname$ on several
  challenging linear examples. Results for KoAT and SPEED were extracted
  from previous publications~\cite{GulwaniJK09,GulwaniMC09,GulwaniZ10,SinnZV14} because KoAT cannot take C programs as
  ...
  marked with --- indicate that the tool failed to produce a result. We write
  $\max(a,b)$ for the maximum of $a$ and $b$.  Functions with names of the form \code{tXX} are complex tests
  that we designed during the development of $\toolname$.  The source code for all the functions
  is available in the extended version~\cite{anon_extended}.
}
\vspace{6pt}
\label{tab:eval}
\footnotesize

\newcommand{\dir}[2]{\code{#1} \\ \code{#2}}
\newcommand{\file}[2][c]{%
...
%--------------------------------------------
% \hline \file{\dir{SPEED\ PLDI09}{fig1}} &
%
% ? &
% &
%
...
%--------------------------------------------
\hline \file{\dir{speed\ pldi09}{fig4\ 2}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ pldi09}{fig4\ 4}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ pldi09}{fig4\ 5}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ pldi10}{ex1}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ pldi10}{ex3}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ pldi10}{ex4}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{fig2\ 1}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{fig2\ 2}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{nested\ multiple}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{nested\ single}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{sequential\ single}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{simple\ multiple}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{simple\ single2}} &
...
%--------------------------------------------
\hline \file{\dir{speed\ popl10}{simple\ single}} &
