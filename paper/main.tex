...
\iffull{}{ The main parts of the soundness proved are formalized with
...
To prove the soundness of \toolname we found it useful to go through an
intermediate step using a Quantitative Hoare Logic.  This logic is at
the same time a convenient semantic tool and a clean way to interface
manual proofs with our automation.  We base it on the one presented in
\cite{veristack14}, we add support for arbitrary resources and simplify
the handling of auxiliary state.

Following this previous work, we define quantitative Hoare triples
...
state $\state'$.  The logic rules are similar to the ones in previous
work and are simply extended to account for all the costs found in our
...
many rules and a thorough overview of our soundness proof.}{Details about
the logic and its soundness proof are provided in the \pref{app:logic}.}

\paragraph{The Soundness Theorem.}
We use the logic from previous paragraph as the target of a translation
...
\caption{ Comparison of resource bounds derived by different tools on
  several examples with linear bounds.  The output of Rank has been manually
...
\emph{t08a}, which is described in~\pref{sec:AAA}.  Example \emph{t19}
demonstrates the compositionaliy of the analysis.  The program consists
...
So in total the program performs $52 + |[-1,i]| + |[0,k]|$ increments.
...
%
In the LOC column
where not only count the LOC of the analyzed function but
also the LOC of all the function it calls. \iffull{The line count is
computed on pre-processed C with all unnecessary declarations removed.}{}
\jan{What is the cost measure?}
...
best possible if the input variable $x$ is non-negative.  \iffull{If $x$ is
...
but has very rudimentary support for automation that cannot
automatically derive symbolic bounds and is not based on efficient LP
solving.  In this article, we use a more general quantitative Hoare
logic that is parametric in the resource of interest.  Our main
contribution is the automatic amortized analysis for Clight that can
be combined with~\cite{veristack14} to automatically derive parametric
verified stack bounds for recursive functions.
...
interactively deriving bounds. In particular, recent
systems~\cite{HoffmannS13} that deal with integers and arrays cannot
derive bounds that depend on values in mutable locations or possibly
negative integers, and thus do not apply to most of our example
programs.
...
techniques with the potential method of amortized analysis.  The
technical innovations are implemented in the publically available tool
...
of symbolic bounds to efficient LP solving.
...
It addresses the longstanding open problem of extending automatic
amortized resource analysis to compute bounds for programs that loop
on singed integers and to deal with non-linear control flow that is
introduced by break and return statements.

This work is the starting point for several projects that we plan to
investigate in the future. Examples include the extension to
concurrent programs, better integration of low-level features like
hardware caches, and the extension of the automatic analysis to
...
opereation modeled by \code{tick(40)} is performed.  Taking the worst
