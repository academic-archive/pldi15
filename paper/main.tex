...
Available techniques split in two categories.  Tools
from the first one derive impressive bounds for
...
There is no opportunity for sound interaction with the
tool, for example, manual proofs cannot be composed
with automatically inferred bounds.  One reason for
this impossibility to interact is that available tools
work on an abstract version of the source program. This
abstracted program has a lot more behaviors than the
original one and is usually expressed in a language that
does not feature memory operations.

In the second category, tools derive very tight
...
analysis (AAA) for Clight programs.  Some example bounds are derived
step by step following our algorithm.
...
like to infer automatically the potential functions.  The first step
...
$$
where $q_{(x,y)} \in \Qplusz$ and $|[a,b]| = \max(b-a,0)$.  We assumed
...
With two concrete examples from open-source projects we will try to
show the reader that compositionality is indeed useful in the analysis
of common code.
