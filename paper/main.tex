...
\code{x=x+1}.  To the best of our knowledge, no other tool is
...
To understand the capabilities of the automatic analysis, it is instructive to
look at some more involved examples.  Examples \emph{speed\_1} and
\emph{speed\_2} are taken from Gulwani et al~\cite{GulwaniMC09}.  They
are both handled by the SPEED tool~\cite{GulwaniMC09} but require
inference of a \emph{disjunctive invariant}.  In the abstract
...
nested loops.  The outer loop is iterated $|[n,0]|$ times.  In the
outer loop, we increment the variable $n$ till $n = 0$.  In each of
the $|[n,0]|$ iterations, we increment the variable $y$ by $1000$.  We
then non-deterministically execute an inner loop that decrements $y$
by $100$ until $y<100$.  The analysis can derive that only the first
execution of the inner loop depends on the initial value of $y$.  We
again derive a tight bound.
...
analysis system can compute a tight bound.  On this example, it
amounts to computing the meeting point of two trains that approach
each other with different speeds.
% This is comparable to the
% determination of the point at which to trains meet if they approach
% each other with different speeds.

Please note that we can only show a very small selections of the
programs that we can handle automatically here.  In \pref{app:cat} you
find a list of more than $30$ classes of challenging programs that we
can automatically analyze.  We hope that this gives you an impression
of the capabilities of the analysis but we also point out that this
...
\Rule{S:BrkLoop}
{}
{ (\state, \code{break}, \Kloop \cont, \cost)
\smallstep
...
\Rule{S:RetLoop}
{}
{ (\state, \code{return}~x, \Kloop \cont, \cost)
\smallstep
(\state, \code{return}~x, \cont, \cost)
}
%
