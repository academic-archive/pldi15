...
\item We describe an automatic static analysis that computes bounds
  ...
  analysis is based on type-based amortized resource analysis for
  functional programs~\cite{Jost03,HoffmannAH12} and computes
  derivations by generating simple linear constraints that can be
  solved by an off-the-shelf LP solver.
...
By compiling these bounds together with the C program, we derive
...
constant resource consumption at each step is parametric in
user-definable cost metric and can be negative to express the release
of resources (as needed, e.g., to reason about memory usage).
Furthermore, a specific \emph{tick} function can be inserted in the
source code to further customize the cost model.  In the
implementation of the logic, we use a shallow embedding in Coq which
makes the logic very flexible.  There is practically no limitation on
the format of the derived resource bounds since they can be any
function that is definable in Coq.  The derived bounds can also be
parametric in a set of cost metrics or specifically apply to one fixed
cost metric.
...
Hoare triples can be used to specify resource cost of library
functions or to describe resource contracts between different
functions.  However, reasoning about quantitative properties can often
be more tedious then reasoning about intentional properties.
Consequently, automation is inevitable to derive resource bounds for
large code bases like CertiKOS.  Such an automation was easily
achievable in the case of constant stack bounds for a code base that
does not contain recursive functions.  In general however, resource
bounds have to be parametric in the arguments of a function and depend
on the number of loop iterations and (recursive) function calls that
are performed by the function.

For functional programs, there exist resource analysis systems that
...
type-based amortized analysis to derive bounds for functions whose
...
abstract--interpretation base methods.  In contrast with abstract
...
prove system that produces verifiable certificates for the bounds.
...
number $A(0)$ of times the test \lstinline{a[i] > m} is true during the
execution.
%
We observe that $A(0)+1$ is the count of ``maximum candidates'' in the
array \lstinline{a} seen by the algorithm. We define
$$
 A(i) =  \# \{ k \mid i \le k < N \land \forall\, 0 \le j < k.\, a[j] < a[k] \} \, .
$$
where we write $\# S$ for the cardinal of the set $S$.  $A(0)$ is
...
But with the expressivity of our  it is possible to use the
...
\end{align*}
%
We can now define the \emph{semantic validity} of a judgement $\cdot; B; R
\vdash \htriple P S Q$ of the quantitative Hoare logic with
empty function context as follows
\begin{align*}
&\valid n B R P S Q := \\
...
XXXXX  Some more explanations.  Where is the difficulty? XXX
...
XXXXX  Some more explanations.  Why n+1?XXX
...
\item
  ...
  to its matching potential in the pre/precondition,
\item
...
materialized in the equation $Q = P + S$.  Arguments in
the function precondition $(\Gamma_f, Q_f)$ are named
using a fixed vector $\Vargs$ of names different from
all program variables.  This prevents name conflicts
from happening and ensures that the substitution
$[\Vargs/\vec x]$ is meaningful.  Symmetrically,
we use the unique name $\Vret$ to represent the
return value in the function postcondition
$(\Gamma'_f, Q'_f)$.
