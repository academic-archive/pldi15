...
with system code and examples from the literature. XXX
...
\section{Amortized Analysis for C Programs}
\label{sec:inform}

\subsection{Amortized Analysis}
The idea that drives the design of our system is amortized analysis
[XXX].  Say that a program $p$ evaluates on a program state $H$ by
consuming $n$ resource units, we note that $(p, H) \Downarrow_n$.  The
basic idea of amortized analysis is to define a potential function
$\Phi$ that maps program states to non-negative numbers, such that
$\Phi(H) \ge n$.
...
simplicity, we use cost metric that counts cost $1$ for assignments
...
The analyzers finds a tight bound.
...
switched using the local variable $t$ as a buffer.  Out analysis
...
prove the termination.  While the SPEED tool has to find a fairly
...
assignments ($2$) and the potential of $|0,vb|$ ($2$).  Since we also
know that $|[0, n]|>0$ we obtain constant potential $4$ and establish
the loop invariant again.  In the \emph{else branch}, we use the
potential $2|[0,vb]|$ and the fact $vb>0$ to obtain $2$ potential
units to pay for the assignment.  Since we have $4$ constant potential
units left after the loop, we can subtract them from the initial
potential $6 + 4|[0, n]|$.
...
size $|[0,i]|$ in the else branch of the conditional since $|[0,i]|$
decreases.  Example \emph{fig\_4\_5} is one of the three examples that
we can not handle automatically.  The reasoning is based on the fact
that the boolean value of the test \code{dir==fwd} does not change
during the iteration of the loop.  As a result, the variable $i$ is
either counted down to $0$ or up to $n$.  Our tool cannot handle
example \emph{fig\_4\_5} because we don't do path sensitive reasoning.
Note however that it would be more efficient to move the test
\code{dir==fwd} outside of the loop (this would be also done by an
optimizing compiler).  The resulting program can be analyzer by our
tool.

Example \emph{ex1} from [GulwaniPLDI10] specifically focusses on the
code in \emph{if} statement.  So we use the \emph{tick metric} and
...
$n=n-1$.  So we can use the potential of the interval size $|[0,n]|$
to pay for the tick.
...
Example \emph{ex2} is similar to example \emph{nested\_single} and
\emph{ex3} is a variant of example \emph{t47}.
...
\code{count\_down}.  For the function $\_main$ that calls
