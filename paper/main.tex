...
\Rule{Q:Break}
{ }
{ (\Gamma, Q_B); R; (\Gamma, Q_B) \vdash \code{break} \dashv (\Gamma', Q') }
%
\and
\Rule[leftskip=.3cm,rightskip=.4cm]{Q:Tick}
{ n < 0 \implies Q \ge 0 }
{ B; R; (\Gamma, Q) \vdash \code{tick}(n) \dashv (\Gamma, Q{-}n) }
%
\and
\Rule{Q:Return}
{ P = Q_R[\Vret/x]
...
{ Q \ge Q'
\\ (\Gamma', Q'); R; (\Gamma,Q) \vdash S \dashv (\Gamma,  Q)
}
...
{ \textstyle
  B; R; (\Gamma[x/x {+} y], Q) \vdash x \gets x + y \dashv (\Gamma, Q')
}
%
\and \Rule{Q:Dec}
...
%
\and \Rule{Q:If}
{ B; R; (\Gamma \land e, Q) \vdash S_1 \dashv (\Gamma',Q')
\\\\ B; R; (\Gamma \land \neg e, Q) \vdash S_2 \dashv (\Gamma',Q')
}
{ B; R; (\Gamma, Q) \vdash \code{if}(e)~S_1~\code{else}~S_2 \dashv (\Gamma',Q') }
%
\and\Rule{Q:Seq}
{  B; R; (\Gamma, Q) \vdash S_1 \dashv (\Gamma', Q')
...
%
\and \Rule[leftskip=.2cm,rightskip=.2cm]{Q:Assert}
{ }
{ B; R; (\Gamma, Q) \vdash \code{assert}~e \dashv (\Gamma \land e, Q) }
%
...
\ifshort{
                    \jan{XXX THIS FIGURE NEEDS TO BE UPDATED XXX}
}{}
...
{ \inferrule*[right={\scriptsize (Q:If)},leftskip=0cm,rightskip=1cm,width=20cm]
  { \inferrule*[right={\scriptsize (Q:Weak)},leftskip=2.5cm,rightskip=2.5cm, vdots=1cm]
    { \inferrule*[right={\scriptsize (Q:Seq)},leftskip=0cm,rightskip=0cm]
      { \inferrule*[right={\scriptsize (Q:Weak)},leftskip=0cm,rightskip=0cm]
        { \inferrule*[right={\scriptsize (Q:Dec)},leftskip=0cm,rightskip=0cm]
          {
          }
          { (\cdot, B^\text{de}); ((x {\geq} 10,Q^\text{de})  \vdash \code{x = x - 10}
            \dashv (x {\ge} 0, P^\text{de})
          }
        }
        { (\cdot, B^\text{we}); ((x {\geq} 10,Q^\text{we})  \vdash \code{x = x - 10}
          \dashv (\cdot, P^\text{we})
        }
        \and
        ...
        { (\cdot, B^\text{ti}); (\cdot,Q^\text{ti})  \vdash \code{tick(5)}
          \dashv (\cdot, P^\text{ti})
        }
      }
      { (\cdot, B^\text{sq}); ((x {\geq} 10,Q^\text{sq}) \vdash
        \code{x=x{-}10; tick(5);} \dashv (\cdot,P^\text{sq})
      }
    }
    { (\cdot, B^\text{if}); ((x {\geq} 10,Q^\text{if}) \vdash
    ...
    { \inferrule*[right={\scriptsize (Q:Break)},leftskip=0cm,rightskip=0cm]
      { }
      { (\cdot, B^\text{br}); (\cdot,Q^\text{br}) \vdash
        \code{break} \dashv (\bot,P^\text{br})
      }
    }
    { (\cdot, B^\text{el}); (x {<} 10,Q^\text{el}) \vdash
      \code{break} \dashv (\cdot,P^\text{el})
    }
  }
  { \hspace{3cm} (\cdot, B^\text{lo}); (\cdot,Q^\text{lo}) \vdash
    \loopbody \dashv (\cdot,P^\text{lo}) \hspace{3cm}
  }
}
{ (\cdot, B); (\cdot, Q) \vdash \code{loop}~{\loopbody} \dashv (\cdot, P)
}
\vspace{-.3cm}
\end{mathpar}
\flushleft
...
& B^\text{we} {=} B^\text{de} \land Q^\text{we} \succeq_{(x{<}10)} Q^\text{de}
    \land P^\text{de} \succeq_{(x{\ge}0)} P^\text{we}
...
\textsc{Relax}. It is parameterized by a logical context
used to gather information on interval sizes.  For example,
$P^\text{de} \succeq_{(x{\ge}0)} P^\text{we} \equiv$
% SSS SPACING
%\vspace{-.3cm}
{\small
\begin{align*}
  p^\text{we}_{0,10} \leq p^\text{de}_{0,10} + u_{0,10} - v_{0,10}
  \land p^\text{we}_{x,0} \leq p^\text{de}_{x,0} + u_{x,0} - v_{0,x} \land \;\;
  \\
  p^\text{we}_{0} \leq p^\text{de}_{0} - 10 {\cdot} u_{0,10} + 10 {\cdot} v_{0.10}
  \bigwedge_{(\alpha, \beta) \neq (0,10), (x,0)} p^\text{we}_{\alpha,\beta} \leq p^\text{de}_{\alpha,\beta} \;.
\end{align*}
}
% SSS SPACING
%\vspace{-.8cm}
...
simplicity, and to compare our analysis with existing tools, we use
tick statements to count the number of loop iterations and function
calls.  This is the only resource that can be measured by tools like
KoAT~\cite{BrockschmidtEFFG14} and LOOPUS~\cite{SinnZV14}.  Sometimes
we also use (negative) tick statements to discuss features such as
