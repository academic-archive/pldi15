...
needed to verify cryptography protocols~\cite{BartheGB09}.
...
hand in order to run them with the tools KoAT and Pubs.  Both tools
and Rank were not able to find linear bounds for the examples.  LOOPUS
is able to derive a bound on XXX of the examples.  But in contrast to
our analysis, the constant factors are not tight.  We also contacted
the authors of SPEED but were not able to obtain a tool to perform a
comparison.
...
\caption{Exemple derivation using amortized reasoning, we noted
  $\#_1(a)$ for $\# \{ i \mid 0 \le i < N \land a[i] = 1 \}$, we use the
...
$\Gamma$ are only used in the assignment \code{x=x+1}.
...
\emph{speed\_2} are taken from Gulwani et al~\cite{GulwaniMC09}.
They are both handled by the SPEED tool~\cite{GulwaniMC09} but
...
automatically derived bounds.  The function \code{count\_down}
...
\end{lstlisting}
\caption{Exemple derivation of a stack usage bound for a binary
