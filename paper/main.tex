...
% these theoretical and practical advantages,
it has been
...
We have implemented the automatic analysis in the tool
...
more than XXX lines of C code from the XXX benchmark suite.
%
...
% Jan: We don't need that. Usually, people don't read it.
...
analysis for Clight programs and demonstrate the technique with
...
The automatic derivation of a resource bound using these potential
functions is best explained by example.  If we use the tick metric
that assigns cost $n$ to the function call \code{tick(n)} and cost $0$
to all other operations then the cost of the following example can be
bounded by $|[x,y]|$.% \footnote{Note that there are no restrictions on
...
\end{lstlisting}
To derive this bound in our amortized analysis, we start with
...
iterations without a cost model) and $K=10$, KoAT computes the bound
$|x| + |y| + 10$, Rank computes the bound $y-x-7$, and LOOPUS computes
the bound $y-x-9$.  Only PUBS computes the tight bound $0.1(y-x)$ if
we translate the program into a term-rewriting system by hand.
...
\subsection{Advantages of the Potential Method }

The potential based approach addresses several challenges in resource
bound analysis. This includes amortized bounds in which individual
loop iterations have different resource consumption, sequential loops
or function calls that require the tracking of size changes of
variables, and nested loops that interact by modifying the same set of
variables.

\paragraph{Challenging Loop Patterns} As mentioned earlier, the
automatic analysis can handle challenging example programs without
special tricks or techniques.  Examples \emph{speed\_1} and
...
\toolname is $\frac{N}{8} |[0,l]|$, but if $N<8$ it becomes $7\frac{8-N}{8}
...
\paragraph{User Guidance for Non-Linear Bounds}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\sectskip
\section{Automated Reasoning with \toolname}
\label{sec:AAA}
\aftersectskip

In this section we present formally the analysis that we
use in our implementation to derive resource bounds for C code.

\paragraph{Linear Potential Functions.}

To find resource bounds automatically, we first need to restrict our
search space.  This is done by focusing on bounds that have a certain
shape.  The general form of \emph{potential functions} that we consider is
$$
...
These judgements are similar to classic Hoare triples in the presence
of the early exit statements \code{break} and \code{return}.  The part
$(\Gamma_B, Q_B)$ is the postcondition used when breaking out of
a loop.  For example, it is set upon entry of looping statements by
the rule {\sc Q:Loop} and used before \code{break} statements by the
rule {\sc Q:Break}.  Similarly $(\Gamma_R, Q_R)$ is the postcondition
used before returning from a function call.  The part $(\Gamma, Q) \vdash
S \dashv (\Gamma', Q')$ of the judgement can be seen as a regular
Hoare triple.  All assertions are split into two parts, the logical
part and the quantitative part.  The quantitative part $Q$ represents a
potential function as a collection of non-negative numbers $q_i$ indexed
by the index set $I$.  The logical part $\Gamma$ is left abstract but
is enforced by our derivation system to respect classic Hoare logic
constraints.
...
and leaves the latter undefined at some index $i$ we will implicitly
assume that $q'_i = q_i$.
...
a[y]$ are simply handled by assigning zero potential to coefficients
...
operations.  These two purposes are easily illustrated on the {\sc Q:Skip}
rule.  This rule reuses its precondition as postcondition.  This is
explained by two facts: first, no resource is consumed by the skip
operation, thus no potential needs to be taken out of the precondition
to pay for the evaluation; second, the program state is not changed by
...
$Q' \le Q$, because some potential is always safely lost.  For the sake
...
Most interesting are the rules {\sc Q:IncP}, {\sc Q:DecP}, and {\sc Q:Inc}
which describe how the potential is distributed after a size change of
a variable.  The rule {\sc Q:IncP} is for increments $x \gets x + y$
...
that can be applied if $y$ is not positive.  The rules are equivalent
in the case where $y=0$.  Finally, the rule {\sc Q:Inc} can be applied
if we cannot deduce any information about $y$.  In the implementation,
the rules for increments and decrements are combined into one syntax
directed rule where the specialized rules take priority over {\sc Q:Inc}.
...
that is defined in the rule {\sc Relax}.  Here, $Q$ and $Q'$ are potential
annotations and $\Gamma$ is a logical context.  The rule {\sc Relax}
deals also with the important task of transferring constant potential
...
proof of \toolname is done by a exhibiting a translation morphism.

\subsection{Cost Semantics for Clight}

To base the soundness proof on a formal ground, we start by defining a
new cost-aware operational semantics for Clight.  CompCert Clight's
...
$M$; a tuple of nine rational numbers and two maps.  Each of these
parameters indicates the amount of resource consumed by a corresponding
step in the semantics.  Resources can be released by assigning a negative
...
Two rules for update and tick follow.
\begin{mathpar}
...
an intermediate step using a Quantitative Hoare Logic.  This logic
is at the same time a convenient semantic tool and a clean way to
interface manual proofs with our automation, as explained in XXX.
The logic is an extension of the one presented in \cite{veristack14},
it adds support for arbitrary resources and simplifies the
handling of auxiliary state.

Following the previous work, we define quantitative Hoare triples
as $\Delta;B;R\vdash\htriple Q S {Q'}$ where $B$, $R$, $Q$, and $Q'$ are
maps from program states to $\Qplusz\cup\{\infty\}$ in order to represent
...
or \code{return} statement.  Additionally, $R$ depends on an extra value
that is returned by the function.  The meaning of the triple $\htriple Q S
{Q'}$ is as follows: If $S$ is executed with starting state $\state$,
the empty continuation \Kstop{} and at least $Q(\state)$ resource units
available then the evaluation does not run out of resources and there
are at least $Q(\state')$ resources left if the evaluation terminates
in state $\state'$.  The logic rules are similar to the ones in previous
work and are simply extended to account for all the costs found in
our cost-aware semantics.

\subsection{The Soundness Theorem}

We translate judgements of the automatic amortized analysis as defined in
\pref{fig:auto} into quantitative Hoare triples.  We define a translation
function $\tr$, such that if a judgement $J$ in the automatic analysis
is derivable, $\tr(J)$ is derivable in the quantitative logic.  By using
$\tr$ to translate derivations of the automatic analysis to derivations
in the quantitative logic we can automatically obtain a certified resource
bound for the analyzed program.
...
S \dashv P'$ in the context $\Delta$ by $\tr(J) :=$
$$
