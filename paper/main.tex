...
and cost $0$ for all other operations.  Of course, the examples can
also be analyzed with any other cost metric.
...
\\[.7\baselineskip]
  ...
  \end{minipage}
   ...
   \caption{Examples from Gulwani et al's SPEED (a)~\cite{GulwaniMC09}}
  \label{fig:cat2a}
\end{figure*}
...
\\[.7\baselineskip]
  ...
  \end{minipage}
   ...
   \caption{Examples from Gulwani et al's SPEED (b)~\cite{GulwaniMC09}}
  \label{fig:cat2b}
\end{figure*}
...
\begin{figure*}[t!]
 \setlength{\progwidth}{.24\linewidth}
  \centering
   ...
   \caption{Examples from [GulwaniPLDI09] and [GulwaniPLDI10]}
  \label{fig:cat2c}
\end{figure*}
...
Our analyzer can derive almost all bounds for programs that have been
described as challenges in the literature on bound generation.  There
are only two types of programs that we were not able to handle:
programs that require \emph{super-linear bounds} and program that
require \emph{path-sensitive reasoning} to derive a bound.  In the
following we list all linear examples from the literature that we
found.  Two of the require path-sensitive reasoning.
