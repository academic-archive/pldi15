...
\begin{abstract}
  ...
  experimental evaluation.  The expressivity of the logic is
  demonstrated by manually deriving customized bounds that are
  tailored to specific algorithms.  A comparison of the automatic
  amortized analysis with other automatic tools on 30 challenging
  examples from the literature and open-source software shows that the
  derived bounds are often more precise.
\end{abstract}
...
$((((1+1)+(n+(-1*i)))+((n+(n*j))+((-1+(-1*j))*i)))+((n+(n*j))+((-1+(-1*j))*i)))+1$ &
$O(n^2)$ &
...
$(((1+1)+n)+((-1+((1/2)*n))+((1/2)*(n*n))))+1$ &
$O(n^2)$ &

$max(n, 0) + max(0,  max(n, 0) * max(0, (n + -1)))$ &
$O(n^2)$ &
...
$max(x, 0) + max(0, (y +  2 * max(x, 0))) + max(0, ( 2 * max(x, 0) + max(y, 0)))$ &
$O(n)$ &
...
$((((1+1)+-1)+(((((-1/2)*y)+((1/2)*(y*y)))+(((-1/2)+y)*x))+((1/2)*(x*x))))+-1)+1$ &
$O(n^2)$ &
...
$max(0, (1000 +  1000 * max(0, -n) + max(0, (y + -99)))) + max(0, -n)$ &
$O(n)$ &
...
$max(0, ( 1000 * max(0, (x - y)) + max(y, 0))) + max(0, (x - y)) + max(0, -x)$ &
$O(n)$ &
