...
\preprintfooter{\iffull{YALEU/DCS/TR-1505\hspace{-.8cm}}{Draft\hspace{.5cm}}}%{Draft}   % 'preprint' option specified.
...
the function call \code{tick(n)} and cost $0$ to all other operations
...
a ranking function if we add the statement
...
a finite number of elements are not zero.  \iffull{As a notable difference from
previous works based on amortized-analysis, we do not require all the coefficients
of our potential function to be positive.  In practice, we will see using
a semantic argument that despite this additional freedom, potential functions
...
\Rule{Q:Break}
{ }
{ (\Gamma; Q_B), R \vdash \htriple{\Gamma; Q_B}{\code{break}}{\Gamma'; Q'} }
%
\and
\Rule{Q:Tick}
{ n < 0 \implies Q \ge 0 }
{ B, R \vdash \htriple{\Gamma; Q}{\code{tick}(n)}{\Gamma; Q{-}n} }
%
\and
\Rule{Q:Return}
{ P = Q_R[\Vret/x]
...
{ B, R \vdash \htriple{\Gamma[x/y]; Q}{x \gets y}{\Gamma; Q'} }
%
\and \Rule{Q:Loop}
{ Q \ge Q'
\\ (\Gamma'; Q'), R \vdash \htriple{\Gamma; Q}{ S }{\Gamma;  Q}
}
{ B, R \vdash \htriple{\Gamma; Q}{\code{loop}~S}{\Gamma'; Q'} }
%
\and \Rule{Q:Inc}
{  \textstyle q'_{0y} = q_{0y} - \sum_u \max(q_{ux}, -q_{xu})
\\\\ \textstyle q'_{y0} = q_{y0} - \sum_u \max(q_{xu}, -q_{ux})
}
{ \textstyle
  B, R \vdash \htriple{\Gamma[x/x {+} y]; Q}{x \gets x + y}{\Gamma; Q'}
}
%
\and \Rule{Q:Dec}
{  \textstyle q'_{0y} = q_{0y} - \sum_u \max(q_{xu}, -q_{ux})
\\\\ \textstyle q'_{y0} = q_{y0} - \sum_u \max(q_{ux}, -q_{xu})
}
{ \textstyle
  B, R \vdash \htriple{\Gamma[x/x {-} y]; Q}{x \gets x - y}{\Gamma; Q'}
}
%
\and \Rule{Q:If}
{ B, R \vdash \htriple{\Gamma \land e; Q}{S_1}{\Gamma'; Q'}
\\\\ B, R \vdash \htriple{\Gamma \land \neg e; Q}{S_2}{\Gamma'; Q'}
}
{ B, R \vdash \htriple{\Gamma; Q}{\code{if}(e)~S_1~\code{else}~S_2}{\Gamma'; Q'} }
%
\and\Rule{Q:Seq}
{  B, R \vdash \htriple{\Gamma; Q}{S_1}{\Gamma'; Q'}
...
{ B, R \vdash \htriple {\Gamma_f[\Vargs / \vec x] \land \Gamma_{\text{Loc}}; Q {+} c}
  {r \gets f(\vec x)}
...
\and \Rule{Q:Assert}
{ }
{ B, R \vdash \htriple{\Gamma; Q}{\code{assert}~e}{\Gamma \land e; Q} }
%
\and \Rule{Q:Extend}
{ \fenv f = (\vec y, S_f)
\\ Q_f \ge 0
...
syntax but instrument the semantics with a resource counter
(an arbitrary rational number) that can be modified by
\code{tick()} function calls.
More details on this cost-aware semantics are provided in \pref{sec:soundness}.
...
that we leave implicit in the rules presented here.
% More details can be found in the extended version.
...
explained by two observations: First, no resource is consumed by the skip
...
The rule {\sc Q:Loop} is a cornerstone of our analysis.  To apply it on a
loop body, one needs to find an invariant potential $Q$ that will pay for
the iterations.  At each iteration, some resources are spent by the loop
body.  Since the  loop can only be exited with a \code{break} statement,
the postcondition $\{ \Gamma'; Q' \}$ for the statement
$\code{loop}~S$ is used as break postcondition in the derivation for $S$.
...
\caption{Assisted bound derivation using logical state.  We write
  $\#_1(a)$ for $\# \{ i \mid 0 {\le} i {<} N {\land} a[i] {=} 1 \}$.
...
\caption{Assisted bound derivation using logical state.
  % We model the stack consumption of the program using tick statements.
...
as $\iffull{\Delta;\,}{}B;R\vdash_L \htriple Q S {Q'}$ where $B$, $R$, $Q$, and $Q'$ are
maps from program states to an element of $\Q\cup\{\infty\}$ that represents
...
the empty continuation, $Q'(\state) \ge 0$ for all state $\sigma$, and at least $Q(\state)$ resource units available
then the evaluation does not run out of resources and there are at
least $Q'(\state')$ resources left if the evaluation terminates in state
...
\iffull{$$
  (\lambda f .\, \tr(\Delta(f))); \tr(B); \tr(R) \vdash_L
    \htriple{\tr(P)}{S}{\tr(P')}.
$$}{%
$$
...
down to $-1$.  However, between the loops we assign \lstinline{i=i+k+50}.
...
We analyzed the functions by giving a cost 1 to all
...
(\pref{sec:AAA}). Second, it is easily combined with established
...
We describe our system for a subset of Clight that is sufficient to
discuss the general ideas.  This subset is given by the following
grammar.
%
\begin{align*}
S &:= \code{assert}~E
...
or the stack through a pointer.
...
stack variables that have their address taken.
...
\code{tick(n)} that can be called with a constant rational number $n$
as a flexible way to model resource consumption or release (if $n$ is
negative).
...
\smallstep
(\state', S_f, \Kcall r \theta \cont, \cost)
}
%
\and
\Rule{S:Loop}
...
% OOPS this is no longer true... This is an argument agains a presentation
% with ticks only. XXX
%
...
{ \Delta; B; R \ldash
\htriple
  { \code{istrue}~\sem{e}_\state \implies Q }
  { \code{assert}~e }
  { Q }
}
%
...
{ \Delta; B; R \ldash
\htriple
  { \lambda \state.\, Q\,\state[x \mapsto \sem{e}_\state] }{ x \gets e }{ Q }
}
%
\and
\Rule{L:Loop}
...
%\\
  \Delta; B; R \ldash
  \htriple
  { \code{istrue}~\sem{e}_\state + P }
  { S_1 }{ Q }
\\
  \Delta; B; R \ldash
\htriple
  { \code{isfalse}~\sem{e}_\state + P }
  { S_2 }{ Q }
}
{ \Delta; B; R \ldash
\htriple
...
\Rule{L:Call}
{
  ...
  P \ge P_f \, y \, (\state(\vec x)) + A \\
  \forall v.\, (Q_f \, y \, v + A \ge \lambda \state.\, Q \, \state[r \mapsto v])
}
{ \Delta; B; R \ldash
\htriple
  { P }
...
\\\\
  ...
  \rightarrow
  \forall y\,\vec v.\,
  (\Delta \cup \Delta'; \bot; Q_f\,y \ldash
...
assertions because calling the \code{tick} function is the only way to
...
is closely related to the number of times the test \code{a[i] > m}
...
Again, we use the function \code{tick} to
...
\pref{fig:wknown} shows well-known algorithms that can be automatically
