...
\begin{abstract}
  ...
  framework that tackles these challenges for C programs.  First, the
  runtime resource cost of programs is formalized with a parametric
  ...
  Coq Proof Assistant.  Third, an automatic amortized resource
  analysis for C programs that computes derivations in the
  quantitative Hoare logic is developed and implemented.  Both, the
...
is the only one that can deal with resource like memory that may
become available during the execution.
...
quantitative Hoare triple $\htriple{P}{S}{Q}$ that $C$ consumes $P(\state)$
resources when executed in starting state $\state$, and that $Q(\state')$
resources are left if $C$ terminates in state $\state'$.  Infinity
...
% different possible ways to exit a block of code.  This is approach is
