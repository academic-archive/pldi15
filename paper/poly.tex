...
empty index represents the constant base polynomial 1.
...
$P$ consumes $n$ resource units turning a state $\state$ into $\state'$,
we note this $\state \rightarrow^n_P \state'$.  The \emph{potential
method} make us look for two annotations $Q$ and $Q'$ such
that
$$
  \forall n\, \state\, \state'.\, \state \rightarrow^n_P \state'
...
satisfy the previous condition when the program $P$ is an
increment operation $x \gets x+y$.

\paragraph{Shifts of Resource Polynomials}

By symmetry we can consider the case where $P=x \gets x+y$ with
$y>0$, the case where $y \le 0$ is only a few signs away.  Let
$Q$ be a potential annotation, we will describe a $Q'$ and a
$P^y_.$ map such that if $\state \rightarrow_P \state'$ (with no resource cost),
